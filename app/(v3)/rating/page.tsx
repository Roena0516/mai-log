...
import type { GameId, Song, RecentLogEntry, DiffType, MarkType } from '../lib/types';
...
export default function RatingPage() {
  ...
  const updateGame = (game: GameId) => {
    ...
    setCtxGame(game);
    const g = GAMES[game];
    const withRS = songs.map((s) => ({ ...s, _rs: g.calcRS(s.ach, s.lv, s.marks) }));
    setConvertedTotal(g.formatTotal(g.calcTotal(withRS, recentLogs)));
  };

  useEffect(() => { if (!loading) updateGame('maimai'); }, [loading]);
  ...
  if (activeGame === 'maimai' || activeGame === 'chunithm') {
    const newSongs = withRS.filter((s) => s.isNewVersion).sort((a, b) => b._rs - a._rs);
    const oldSongs = withRS.filter((s) => !s.isNewVersion).sort((a, b) => b._rs - a._rs);
    const [newTop, oldTop] = activeGame === 'maimai' ? [15, 35] : [20, 30];

    const renderSection = (list: SongWithRS[], top: number) =>
      list.slice(0, top).map((s, i) => (
        <SongRow key={`${s.name}-${s.diff}-${s.isDx}`} song={s} game={activeGame} rank={`#${i + 1}`} />
      ));
  ...
  return (
    <div>
      ...
      {recentTop10.map((s, i) => (
        <SongRow key={`recent-${s.name}-${s.diff}-${s.isDx}`} song={s} game={activeGame} rank={`#${i + 1}`} />
      ))}
      <SectionHeader label="BEST" count={30} />
      {best30.map((s, i) => (
        <SongRow key={`best-${s.name}-${s.diff}-${s.isDx}`} song={s} game={activeGame} rank={`#${i + 1}`} />
      ))}
    </div>
  );
}
